...
#------------------------------DEFINIING THE FUNCTION FOR CALCULATING SIMPLE MOVING AVERAGE-----------------------------
...
#------------------------------PLOTS THE DATA_FRAME---------------------------------------------------------------------
def Data_Frame_plot(Data_Frame):
    Data_Frame.plot()
    plt.show()

#----------------------------- MAIN FUNCTION WITH DATAFRAME AND TRADING ALGO IS CALLED WITH FILE NAME-------------------
def MVA_DF(File):
...
#-------------------Incase OF difference between BUY and Sell arry length.
    list_0 = len(Buy_list)#---counting thw number of value in list_o
    list_1 = len(Sell_list)#---counting thw number of value in list_1
    ...
    if list_1==list_0:
        ...
        for new_value in Sell_list_list:    #if both list are not equal
            if(list_1 < list_0):
                x = Sell_list_list[list_1-1]
                list_1 = list_1+1
                Sell_list_list.append(x)     #update sell1 list
            else:
                print("SHORT SELLING")

    print(len(Sell_list_list))

    quant = 200       #assigened quantity
    ...
    def buy(i, j):  # Funcation for buy
        ...
        for update in trade_cost_buy:
            avl_balance = amount - trade_cost_buy[a]        #counting the avl_balance
            amount = avl_balance
            # amount=avl_balance
